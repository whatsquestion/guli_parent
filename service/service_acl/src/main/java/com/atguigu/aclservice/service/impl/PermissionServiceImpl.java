...
//        });
    ...
    public List<Permission> getAllMenu() {
        ...
        QueryWrapper<Permission> wrapper= new QueryWrapper<>();
        wrapper.orderByDesc("id");
        List<Permission> allPermissons = baseMapper.selectList(wrapper);

        //将查询的所有权限存储在map集合中，不需要再去查询数据库
        Map<String ,Permission> permissionMap= new HashMap<>();
        allPermissons.forEach( permission -> {
            permissionMap.put(permission.getId(), permission);
        });

        List<Permission> rootPermissons= new ArrayList<>();
        allPermissons.forEach(permission -> {
            if ("0".equals(permission.getPid())){
                permission.setLevel(1);
                rootPermissons.add(permission);
            }else {
                Permission parentPermisson = permissionMap.get(permission.getPid());
                parentPermisson.getChildren().add(permission);
            }
        });
